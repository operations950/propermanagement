...
def _derive_site_base_url(origins):
    """The app's own public address, for links in messages sent when no web
    request is in flight (e.g. a recurring visit generated by the scheduler
    that texts its cleaner a link). SITE_BASE_URL wins when set; otherwise
    the first real host in ALLOWED_HOSTS (a wildcard entry can't be turned
    into an address)."""
...
SITE_BASE_URL = (os.environ.get('SITE_BASE_URL') or _derive_site_base_url(CSRF_TRUSTED_ORIGINS)).rstrip('/')
