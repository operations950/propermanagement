...
from processes.models import ProcessInstance, ProcessInstanceDocument, ProcessInstanceItem, ProcessTemplate
...
def ticket_detail(request, pk):
    ...
    return render(request, 'tickets/ticket_detail.html', {
        ...
        'process_instances': ticket.process_instances.select_related('process_template').prefetch_related(
            'items__attachments', 'items__document',
        ),
...
@login_required
def process_attach(request, pk):
    """Attaches one or more ProcessTemplates to this ticket — copies each
    template's items onto a new ProcessInstance (snapshot, not a live
    reference, same rationale as generate_recurring_tickets' template->
    instance copy). Ticket can't close until every attached instance is
    fully checked (see tickets/services/process_gate.py)."""
    ticket = get_object_or_404(Ticket, pk=pk)
    if request.method == 'POST':
        ...
        for template in ProcessTemplate.objects.filter(pk__in=template_ids, is_active=True):
            instance = ProcessInstance.objects.create(process_template=template, ticket=ticket)
            for item in template.items.all():
                ProcessInstanceItem.objects.create(
                    instance=instance, text=item.text, sequence_order=item.sequence_order,
                    is_required=item.is_required, requires_upload=item.requires_upload,
                    action_type=item.action_type, document_key=item.document_key,
                )
...
@login_required
def process_item_toggle(request, item_pk):
    """Toggles one ProcessInstanceItem's checked state — mirrors
    ticket_checklist_toggle, but a requires_upload item can't be checked
    until at least one ProcessAttachment already exists for it."""
    item = get_object_or_404(ProcessInstanceItem, pk=item_pk)
    if request.method == 'POST':
        if not item.is_checked and item.requires_upload and not item.attachments.exists():
            messages.error(request, 'Attach a file to this step before checking it off.')
        else:
            item.is_checked = not item.is_checked
            item.checked_at = timezone.now() if item.is_checked else None
            item.checked_by = request.user if item.is_checked else None
            item.save(update_fields=['is_checked', 'checked_at', 'checked_by'])
    return redirect('ticket_detail', pk=item.instance.ticket_id)


@login_required
def process_item_upload(request, item_pk):
    """Saves a proof-of-completion upload against one ProcessInstanceItem
    (e.g. the signed affidavit, a photo of the physically-posted notice)."""
    item = get_object_or_404(ProcessInstanceItem, pk=item_pk)
    if request.method == 'POST':
        form = ProcessAttachmentUploadForm(request.POST, request.FILES)
        if form.is_valid():
            attachment = form.save(commit=False)
            attachment.instance_item = item
...
@login_required
def process_item_schedule_meet(request, item_pk):
    """Schedules a Google Meet for a 'google_meet' checklist item on the
    logged-in staff member's own connected Google Calendar (there's no
    shared company calendar wired for writes today — see
    core/google_calendar.py). Extracts the Meet link/dial-in onto the item
    and auto-checks it on success; scheduling the meeting IS the completion
    of this step."""
    from core.google_calendar import GoogleCalendarWriteError, create_event
    from core.models import GoogleCalendarToken

    item = get_object_or_404(ProcessInstanceItem, pk=item_pk)
    ticket = item.instance.ticket
    token = GoogleCalendarToken.objects.filter(staff=getattr(request.user, 'staff_profile', None)).first()
    if not token:
        messages.error(request, 'Connect your Google Calendar first (see your department dashboard) to schedule a meeting.')
        return redirect('ticket_detail', pk=ticket.pk)

    if request.method == 'POST':
        raw_datetime = request.POST.get('meeting_datetime', '')
        start = parse_datetime(raw_datetime)
        ...
        try:
            event = create_event(
                token, calendar_id, summary=f'{item.instance.process_template.name} — {ticket.title}',
                start=start, end=end, add_meet=True,
            )
        ...
        item.meeting_datetime = start
        item.meeting_link = video_link
        item.meeting_dial_in = dial_in
...
def _board_meeting_notice_initial(item):
    """Prefills the Notice document from the sibling google_meet item's
    scheduled date/time/link and the ticket's property — see
    tickets/services/process_gate.py's docstring and this feature's plan
    for why this is concrete, named lookups rather than a generic
    variable engine."""
    ticket = item.instance.ticket
    prop = ticket.property
    meet_item = item.instance.items.filter(action_type='google_meet').first()
    when = (
        meet_item.meeting_datetime.strftime('%B %d, %Y at %I:%M %p')
        if meet_item and meet_item.meeting_datetime else '[enter date/time]'
    )
    lines = [
        'NOTICE OF BOARD MEETING',
        prop.name if prop else '[property name]',
        '',
        f'Date/Time: {when}',
        f'Location: {prop.board_meeting_address if prop and prop.board_meeting_address else "[enter meeting location]"}',
    ]
    if meet_item and meet_item.meeting_link:
        lines.append(f'Join by video: {meet_item.meeting_link}')
    if meet_item and meet_item.meeting_dial_in:
        lines.append(f'Join by phone: {meet_item.meeting_dial_in}')
    lines += [
        '',
        'Agenda:',
        '1. Call to order',
        '2. [add agenda items]',
        '',
        'Quorum required: [enter quorum requirement from the association\'s governing documents]',
        '',
        'This notice is posted in accordance with Florida Statutes 718.112 / 720.306. Board meetings '
        'must generally be posted conspicuously at least 48 hours in advance; a longer, mailed 14-day '
        'notice (also posted 14 days) is required instead if a special assessment or a rule amendment '
        'on unit/parcel use will be considered. Verify against this association\'s bylaws.',
    ]
    return '\n'.join(lines)


def _board_meeting_affidavit_initial(item):
    ticket = item.instance.ticket
    prop = ticket.property
    meet_item = item.instance.items.filter(action_type='google_meet').first()
    when = (
    ...
    lines = [
        ...
        prop.name if prop else '[property name]',
        '',
        f'I, [enter name], being duly sworn, depose and state that notice of the board meeting '
        f'scheduled for {when} was provided as follows:',
        '',
        'Method of notice: [posted conspicuously / mailed / emailed / hand delivered]',
        'Date and time notice was posted/sent: [enter date/time]',
        'Location posted (if applicable): [enter location]',
        '',
        'This affidavit is executed in accordance with Florida Statutes 718.112 / 720.306.',
        '',
...
# Concrete registry, not a generic templating engine — see this feature's
# plan for why. Add an entry here (plus a ProcessTemplateItem with a
# matching document_key) for each new document-template step.
DOCUMENT_BUILDERS = {
    'board_meeting_notice': {'label': 'Meeting Notice', 'build_initial': _board_meeting_notice_initial},
    'board_meeting_affidavit': {'label': 'Affidavit of Notice', 'build_initial': _board_meeting_affidavit_initial},
}


@login_required
def process_item_document(request, item_pk):
    """Prefilled, editable document for a 'document_template' checklist
    item (Notice / Affidavit) — saved as a ProcessInstanceDocument, printed
    via the browser's own print-to-PDF if a hard copy is needed (no
    PDF-generation dependency, per this feature's agreed scope)."""
    item = get_object_or_404(ProcessInstanceItem, pk=item_pk)
    ticket = item.instance.ticket
    builder = DOCUMENT_BUILDERS.get(item.document_key)
    if not builder:
        raise Http404('No document template configured for this step.')
    ...
    if request.method == 'POST':
        ...
        if doc:
            ...
            doc = ProcessInstanceDocument.objects.create(instance_item=item, content=content, generated_by=request.user)
        item.is_checked = True
        item.checked_at = timezone.now()
        item.checked_by = request.user
        item.save(update_fields=['is_checked', 'checked_at', 'checked_by'])
        messages.success(request, f'{builder["label"]} saved.')
        return redirect('ticket_detail', pk=ticket.pk)

    content = doc.content if doc else builder['build_initial'](item)
    return render(request, 'tickets/process_document.html', {
        'ticket': ticket,
        'item': item,
        'label': builder['label'],
        'content': content,
